...
type
  Tfrm_Proj = class(TForm)
    ...
    BindSource_Proj_Não_: TBindSourceDB;
    FDTable_Proj_Não_: TFDTable;
    ...
    procedure FDTable_Proj_Não_BeforePost(DataSet: TDataSet);
...
procedure Tfrm_Proj.FDTable_Proj_Não_BeforePost(DataSet: TDataSet);
begin
//FDTable_Proj_Não_.FieldByName('id_produto').value := txt_id.Text;
//FDTable_Proj_Não_.FieldByName('cliente').value := txt_cliente.Text;
//FDTable_Proj_Não_.FieldByName('descricao').value := txt_desc.Text;
FDTable_Proj_Não_.FieldByName('proj').value := txt_proj.Text;
end;

procedure Tfrm_Proj.FormClose(Sender: TObject; var Action: TCloseAction);
begin
FDTableProj_SIM.Close;
FDCon_Proj_SIM.close;

FDTable_Proj_Não_.close;
FDCon_PCP_Nao.close;
...

{
  ...
  begin
     ...
     DB_Exrcutado.Fields[3].Text := 'Não';
